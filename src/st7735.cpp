...
ST7735::ST7735 (int bus, int cs,int frequency, int rs, int rst)
    : GFX (131, 130, m_map, font), m_spi(bus,cs),_frequency(frequency),
...
ST7735::initModule () {
    ...
    error = m_spi.frequency(_frequency);
    if (error != mraa::SUCCESS) {
        mraa::printError (error);
    }
    error = m_spi.mode(mraa::SPI_MODE3);
}
...
ST7735::refresh () {
    ...
    int fragmentSize = m_height * m_width * 2 / 20.0;
