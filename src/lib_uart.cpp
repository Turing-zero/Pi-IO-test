...
void uart_module::open_uart(std::string uart_port_str){
    // init uart
    try {
        if (_port < PI_UART_NUM - 1) {
            uart = new mraa::Uart(devices::dev_path[_port]);
        } else if (_port == CUSTOM) {
            uart = new mraa::Uart(uart_port_str);
        }
...
int uart_module::recv_packet(char *packet, int max_size, int timeout_ms) {
    char *temp = packet;
    int length = 0;
    int wait_count = 0;
    ...
    int64_t finish = start + timeout_ms * 1000;
...
void uart_module::clear_rxbuf(int max_size) {
    char temp;
...
unsigned short uart_module::update_crc(unsigned short crc_accum, unsigned char *data_blk_ptr, unsigned short data_blk_size) {
    unsigned short i, j;
