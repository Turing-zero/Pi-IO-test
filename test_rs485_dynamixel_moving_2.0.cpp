...
int main(int argc, char **argv) {
    ...
    while (flag) {
        ...
        char tx_buf[] = {0xFF,0xFF,0xFD,0x00,0x01,0x09,0x00,0x03,0x40,0x00,0x01,0x00,0x00,0x00};//reg-write
        char tx_buf_d[] = {0xFF,0xFF,0xFD,0x00,0x01,0x09,0x00,0x04,0x74,0x00,0x00,0x00,0x00,0x00};//reg-write
        char tx_buf_a[10] = {0xFF,0xFF,0xFD,0x00,0x01,0x03,0x00,0x05,0x02,0xCE};
        // char tx_buf[25] = {0xFF,0xFF,0xFD,0x00,0x01,0x07,0x00,0x02,0x00,0x00,0x96,0x02,0x27,0x29};
        // tx_buf[0] = 0xab;
        // tx_buf[1] = index & 0xff;
        // tx_buf[2] = (index >> 8) & 0xff;
        int before_crc_bits = sizeof(tx_buf);
        unsigned short CRC_result = update_crc(0,(unsigned char*)tx_buf,before_crc_bits);
        ...
        unsigned short CRC_result_d = update_crc(0,(unsigned char*)tx_buf_d,before_crc_bits_d);
        ...
        // unsigned short CRC_result_a = update_crc(0,(unsigned char*)tx_buf_a,before_crc_bits_a);
        // tx_buf_a[before_crc_bits_a] = 0x02;//CRC_result_a&0xff;
        // tx_buf_a[before_crc_bits_a+1] = 0xCE;//CRC_result_a>>8&0xff;

        
        // std::cout<<int(tx_buf_a[before_crc_bits_a])<<" "<<int(tx_buf_a[before_crc_bits_a+1])<<std::endl;
        // std::cout<<int(tx_buf[before_crc_bits])<<" "<<int(tx_buf[before_crc_bits+1])<<std::endl;
        ...
        if(send_once){
            ...
            // std::cout<<sizeof(tx_buf)<<std::endl;
